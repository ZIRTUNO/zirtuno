...
> ### Update v1.7 (2026-06-11) — the gooey cursor + the ball-bridge melt restored (owner correction pass)
> Owner verdict on v1.6 live: forms not fluid enough at rest, and the hover was "just the canvas leaning toward the cursor — bad." Target feel: the react-bits MetaBalls demo (the §1 engine): gooey cursor interaction + traveling-droplet fluidity. Two spec amendments (owner decisions):
> 1. **The react-bits cursor ball is IN for the hero** — supersedes §1's "not used to deform the forms." The cursor physically merges with the liquid; the canvas lean is REMOVED (the cursor IS the hover; ≤1 % parallax was authorised, zero shipped).
> 2. **The melt returns to the locked v1.2 design**: glass-SDF(A) → 48-ball metaball bridge (min-travel matching, left-to-right stagger 0.25, radius-leads-position 1.18 — §3.3, recovered from the v1.4 build) → glass-SDF(B). The v1.6 SDF-blend/pinch melt read as a crossfade and is retired as the transition; the v1.6 **domain warp survives as an aliveness layer** on rest + melt (`SDF_WARP_REST` raised to 0.0055).
>
> **Implementation — the unified liquid field** (`sdf-glass-shader.mjs`; glass lighting untouched): everything is ONE normalised field summed before a shared surface, `T = iFormA·S(dA) + iFormB·S(dB) + Σ ρᵢ²/|p−cᵢ|²`, where `S(d) = (1 + d/GOO)⁻²` maps a form's signed distance onto the same inverse-square profile as a metaball. T is inverted back to a pseudo-distance `d′ = GOO·(T^−½ − 1)` for the locked shading — with no balls near, d′ ≡ d exactly, so **rest stays pixel-exact by construction** (rest-forms sheet re-verified after the change).
> - **Cursor** = a spring-lagged droplet chain in the same field (`CURSOR_R` + `CURSOR_TRAIL_N` trailing droplets; `CURSOR_SMOOTH` ≈ react-bits hoverSmoothness, frame-rate corrected): it visibly bulges, necks into and pulls on the form's edge, and detaches by easing its radius to 0 (≤300 ms — never pops). **Guardrails:** every ball's field is windowed to `SDF_BALL_REACH ×` its radius (bounded influence); the additive field can only bulge the form outward, never erode it; `CURSOR_INFLUENCE_MARK < 1` so the logo deforms least; off on touch devices and reduced motion.
> - **The melt handoff is in-field, not a crossfade**: form A's weight drains over the first `BRIDGE` window while the droplets condense on its footprint, travel, and fuse into B as its weight rises — one iso-surface the whole time, so the v1.4 crossfade double-exposure is structurally impossible. Mid-frames verified at m ∈ {0.12, 0.3, 0.5, 0.7, 0.88} (both handoffs + travel), all 8 transitions.
> - **QA:** `?fcursor=x,y` freezes a merged cursor droplet on any still (`capture-cursor-merge.mjs` → the bulge → neck → merge sheet, both locales); `?fpair` freezes a bridge frame again; endpoints re-registered against the current SVGs (`generate-morph-endpoints.mjs` output byte-identical — already registered). Keyboard melts, aria-live, and the reduced-motion static mark are unchanged.
