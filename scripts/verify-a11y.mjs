...
// ── i18n parity (pure node) ───────────────────────────────────────────────────
{
  ...
  const pt = new Set(walk(JSON.parse(fs.readFileSync("lib/i18n/messages/pt.json", "utf8"))));
  const en = new Set(walk(JSON.parse(fs.readFileSync("lib/i18n/messages/en.json", "utf8"))));
  ...
  check(
    ...
    [onlyPt.length && `pt-only: ${onlyPt.slice(0, 3)}`, onlyEn.length && `en-only: ${onlyEn.slice(0, 3)}`]
      .filter(Boolean)
      .join(" · "),
  );
}
...
for (const locale of ["pt", "en"]) {
  ...
  await page.goto(`${BASE}/${locale}?ftier=none`, { waitUntil: "domcontentloaded" });
  ...
  const sem = await page.evaluate(() => {
    ...
    for (const h of hs) {
      ...
      if (prev && lv > prev + 1) skip = `${h.tagName} after h${prev}: "${h.textContent.slice(0, 30)}"`;
      prev = lv;
    }
  ...
  check(sem.lang.startsWith(locale), "document lang matches the locale", sem.lang);
  ...
  check(first.cls?.includes("skip-link"), "skip link is the first tab stop", JSON.stringify(first));
  ...
  check(focusVisible === true, "focus is visible on the chrome", String(focusVisible));
  ...
  const contrast = await page.evaluate(() => {
    ...
    let at = "";
    let atL = "";
    for (const el of document.querySelectorAll("h1,h2,h3,h4,p,li,a,span,button,label,dt,dd")) {
...
// ── mobile menu keyboard (one locale suffices — shared chrome) ────────────────
{
  ...
  if ((await burger.count()) === 1) {
    ...
    check(open.expanded === "true" && open.menu, "menu opens by keyboard", JSON.stringify(open));
    await page.keyboard.press("Escape");
    await page.waitForTimeout(500);
    const closed = await page.evaluate(
      () => document.querySelector(".burger")?.getAttribute("aria-expanded"),
    );
    check(closed === "false", "menu closes on Escape", `aria-expanded=${closed}`);
  } else {
...
await browser.close();
console.log(failures === 0 ? "A11Y/I18N: all green" : `A11Y FAILURES: ${failures}`);
