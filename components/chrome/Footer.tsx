...
// TODO(decision): only owner-approved public channels render, same gate the
...
/**
 * The marks, drawn to the reference footer's own geometry (upsunday.co,
 * `.foot__social-link svg`): Instagram as a 1.8-weight outline, everything
 * else as the platform's official solid glyph. That mix is the reference's,
 * not an oversight — the solid marks are each brand's supplied artwork and
 * only Instagram publishes an outline form.
 *
 ...
 * touches. Built as ONE raised glass panel inset from the page edges, so the
 * document visibly ends on an object rather than trailing off into the black.
 *
 * WHY THE PANEL STOPS SHORT OF THE PAGE BOTTOM: `lib/webgl/scenes/footer.ts`
 * anchors the coda on `.footer` and reads its bottom edge — one droplet
 * detaches from the held contact mark and sinks PAST that edge, out of the
 * page. The gutter below the panel is where it lands. A panel flush to the
 * page bottom would swallow the last beat of the whole journey, so the
 * element keeps its `.footer` class and its bottom padding is load-bearing,
 * not decoration.
 *
