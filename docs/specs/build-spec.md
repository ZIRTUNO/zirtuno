...
`docs/specs/metaball-morph-spec.md` for exact liquid mechanics and `docs/references.md` for
...
`docs/specs/metaball-morph-spec.md`. Git history retains the discarded implementation
...
- Integration's circuit encloses a real hole (`scripts/probe/hole.mjs`), and the
...
- one uv unit is the SMALLER viewport dimension, so the narrow stage's room
  must be measured (`scripts/probe/bbox.mjs`), not derived from the landscape
  mapping;
