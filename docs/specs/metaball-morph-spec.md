...
`docs/specs/build-spec.md` owns the visitor experience and chapter acceptance.
...
### 5.7 Services volume and moving handoffs

All seven live handoffs — THE CROSSING out of THE CONFLUENCE into pillar 1,
then Services pairs 1→2 through 6→7 — use the measured density curves in
`lib/webgl/melt-volume.data.mjs`, generated by
`node scripts/tools/fit-melt-volume.mjs`. Each curve budgets the **rendered
field area** against its two exact endpoints. The forms differ in area, so
the budget follows their ramp rather than forcing constant size. Monotone
cubic interpolation preserves continuous derivatives without a stop at every
sample.

THE CROSSING is keyed `cross-1`, not `0-1`, and that key is why the table is
resolved through a function instead of by indexing `CLOUDS`. S3 has no vector
behind it, so this melt has no form A: at p = 0 the droplets ARE the whole
picture, and the curve opens at 1 rather than at 0. Its staging — form B alone,
`CONFLUENCE_STAG`, `matchClouds` correspondence — is unchanged signed-off
choreography; only its density is measured now instead of borrowed. Before the
fit it was the one handoff on the page still running the shared symmetric
envelope, because the lookup keys on the source cloud and THE CONFLUENCE is not
a `CLOUDS` member, so it could not hit at any p. That envelope does not hold
here: `fit-melt-volume.mjs` records the confluence body at ~70% of the web
form's area, a real ~43% growth ramp the shared curve does not model, and a
rendered area that bounced 99 → 120 → 95 → 100% across the drain while mass
detached and re-merged — the loose-speck report. The departure into Método
keeps its existing choreography.

Automation's returning arc (3→4) carries a small density shoulder so its thin
skeleton does not reappear apart from the body; connectivity is checked by the
shape and islands gates, separately from total area. **OPEN — the shoulder is
too local at its present width.** It is a Gaussian of amplitude 0.30 and σ 0.025
centred on p = 0.875, and it lifts the rendered area 23,790 → 25,535 → 23,122 px
(320², 33 steps) across the frames either side of the peak, which
`melt-mass --gate` reads as an 11% single-frame step against the 10% Services
budget. The other five Services pairs measure ≤ 3%. Because the shoulder is a
post-solve multiplier, widening it does not require re-deriving the underlying
curve — but the re-tune must be re-checked against `melt-shape`'s pop ceiling
(3→4 already sits at 0.0339 of 0.0350) and `melt-islands`, and reviewed on the
real scrolling capture. That tune is not in this change.

The live scene also filters the four form weight/erosion channels with
`dampFormPhase`, using the conductor's existing `PHYS.TAU_RADIUS`. Filtering
only progress is insufficient: nonlinear erosion then runs ahead of the
already-filtered droplets and stacks a solid destination onto liquid still
draining. No droplet integration, bind behavior, correspondence, renderer,
endpoint asset, or rest material changes.

On narrow stages the empty `.pillar-stage` grid row precedes the copy so
the form has the space above it that the scene reserves. The scene uses
the same 60rem column breakpoint as `.pillar`, rather than aspect ratio;
tall tablets must not render a centred form over a two-column layout.

Gates:

- `scripts/tools/melt-mass.mjs --gate` holds pairs 1→2 … 6→7 to 1% dip, 1% bump
  and a 10% single-frame step. Its `a = 0` row is NOT the crossing: it is the
  legacy `CLOUDS[0]` → `CLOUDS[1]` mark→web melt the page has not run since THE
  CONFLUENCE replaced the mark, kept under the older 12/24/36 budget as the entry
  vocabulary's own reference. The crossing has no melt-mass row.
- `scripts/tools/melt-shape.mjs` retains the spatial pop, roundness, and
  connectivity limits. `DETAIL=1` localises any frame whose pop exceeds 0.025,
  reporting how much of the fresh liquid landed far from the existing body.
- `scripts/tools/melt-islands.mjs` is where the crossing IS measured. For `a = 0`
  it too used to run `CLOUDS[0]` → `CLOUDS[1]`, so the one melt whose droplets
  carry the frame unshielded was the one melt this probe never looked at; it now
  runs the shipped staging. It reports; it does not exit non-zero.
- `scripts/probe/melt-motion.mjs --gate` exercises the six form → form pairs
  forwards and backwards through the actual form filter and the conductor's
  density/radius/position low-passes. Its two-second traversal bounds swelling
  at 2% and thinning at 12%. It does not cover the crossing.

These are simulation measurements; review the real scrolling capture as well.

`forms:melts` measures the seven live pillar centres and captures all six
bridges using wheel input. `WIDTH`, `HEIGHT`, `MIDS`, and `OUT` configure
desktop/mobile review without changing the page. The deterministic rest
gate accepts `REST_REFERENCE` for a same-machine before/after snapshot;
the committed rest fixture must never be silently replaced.
...
compares SHA-256 against `scripts/fixtures/rest-exact.json`. `forms:rest` remains the
