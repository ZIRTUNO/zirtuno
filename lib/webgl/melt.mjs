...
import { CLOUDS, N, clamp01, hash, smooth01 } from "./phys.mjs";
 ...
 */
export const FORM_SOLIDITY = [0.277, 0.313, 0.276, 0.401, 0.433, 0.302, 0.474, 0.389];
...
export function meltDroplet(out, i, A, B, perm, stag, p, swA = 0, swB = 0, presOverride) {
  ...
  const pres =
    presOverride === undefined ? bridgeDensity(bridgePresence(p)) : presOverride;
...
/**
 * THE MORPH'S SATURATION CEILING at progress p — the one term that bounds how
 * far a crowd of droplets may push the iso-surface outward, and the whole of
 * the inflation fix.
 *
 * It rides the morph's own envelope, dividing rather than scaling: at the
 * peak the ceiling is MELT_SAT, and as the envelope closes it grows without
 * bound, which IS the historical plain sum. So both endpoints of every morph,
 * every rest plateau and every frame of the rest of the page are the original
 * field exactly — the correction exists only across the frames that were
 * inflating, and arrives and leaves continuously.
 *
