...
import { originAcceleration } from "./origin-field.mjs";
    ...
    while (acc >= FLUID.H_MS && steps < maxSteps) {
      ...
      for (let i = 0; i < POP; i++) {
        ...
        const roam = temperOn
          ...
          : 1;
        const basinGain = (env.basin?.gain || 0) * free;
        const leash = LEASH_R * R[i] * free * roam + 0.085 * basinGain;
        let ax;
        let ay;
        if (leash > 1e-5) {
          ...
          const station = 1 - 0.82 * basinGain;
          ax = om * om * dtx * over * station - damp * V[ix];
          ay = om * om * dty * over * station - damp * V[ix + 1];
        } else {
        ...
        if (free > 0.01 && R[i] >= 0.0012) {
          ...
          A2[0] = 0;
          A2[1] = 0;
          if (basinGain > 0) originAcceleration(x, y, t, env.basin, A2);
          cursorAccel(x, y, env);
