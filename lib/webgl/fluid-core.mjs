...
export const FLUID = {
  ...
  FORM_TOUCH: 0.018, // peak hover displacement of the form surface (uv)
  FORM_SHOCK: 0.032, // peak strike displacement of the form surface (uv)
};
...
export function makeFluidCore(opts = {}) {
  ...
  const formUniforms = (tMs, env, touchOut, shockOut) => {
    ...
    touchOut[3] = env.pon
      ? FLUID.FORM_TOUCH * (1 + FLUID.CURSOR_PRESS * (env.press || 0))
      : 0;
    ...
    for (let k = 0; k < SHK; k++) {
      ...
      shockOut[o + 1] = ky[k];
      shockOut[o + 2] = front;
      shockOut[o + 3] = FLUID.FORM_SHOCK * kAmp[k] * (1 - p) * (1 - p) * spread;
      live = true;
    }
    return live;
  };
