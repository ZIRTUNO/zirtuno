...
// 20, not 12: S7 puts its two idea words, its two clauses and its thesis on
// the stage as reading surfaces the vapour must turn aside from, and only the
// surfaces on screen are counted per frame, so the ceiling costs nothing
// outside the chapter that needs it.
export const FLUID_OBSTACLE_MAX = 20;
