...
export function makeConductor(scenes, opts = {}) {
  ...
  const env = {
    ...
    obstacleCount: 0,
    basin: { cx: 0.5, cy: 0.62, gain: 0, capture: 0 },
  };
  ...
  const frame = (tMs, buf, aspect, zBuf, idBuf, dBuf) => {
    ...
    // Scene-authored pressure shares the same ownership blend as composition.
    // No influence leaks into other chapters or into bind=1 trajectories.
    env.basin.gain = 0;
    for (let si = 0; si < scenes.length; si++) {
      if (W[si] < 1e-4 || !scenes[si].dynamics) continue;
      const basin = scenes[si].dynamics(ctxs[si]);
      env.basin.cx = basin.cx;
      env.basin.cy = basin.cy;
      env.basin.capture = basin.capture;
      env.basin.gain = basin.gain * W[si];
    }
    ...
    if (haveW) {
      ...
      if (moteTab) {
        ...
        for (let i = N; i < POP; i++) {
          ...
          // Most scenes inherit the host station. S7 spreads those same ranks
          // across the catchment. Blend from the ordinary station at seams;
          // positions and velocities remain continuous, never reseeded.
          for (let si = 0; si < scenes.length; si++) {
            const w = W[si];
            if (w < 1e-4 || !scenes[si].population) continue;
            tmp.d = 1;
            scenes[si].population(i, ctxs[si], tmp);
            TGT[i * 2] += (tmp.x - TGT[i * 2]) * w;
            TGT[i * 2 + 1] += (tmp.y - TGT[i * 2 + 1]) * w;
            TR[i] += (tmp.r - TR[i]) * w;
            BIND[i] += (tmp.bind - BIND[i]) * w;
            Z[i] += (tmp.z - Z[i]) * w;
            D[i] += (tmp.d - D[i]) * w;
            if (w > 0.72) CLUS[i] = tmp.cluster;
          }
          const ar = TR[i];
          if (R[i] < 0) R[i] = ar;
          else R[i] += (ar - R[i]) * krM;
