...
import { makeMoteTable, moteTarget, moteTauScale } from "./motes.mjs";
import { SDF_WARP_REST, SDF_BALL_MAX } from "./sdf-glass-shader.mjs";
import { MIST_HOSTS } from "./mist-shaders.mjs";
import {
  FLUID,
  FLUID_OBSTACLE_MAX,
...
export function makeConductor(scenes, opts = {}) {
  ...
  // ── R6 population ──────────────────────────────────────────────────────────
  // N is the AUTHORED droplet count and stays exactly that: what every form is
  // packed to, what the scenes address, what the melts move. POP is the
  // SIMULATED count. Droplets N..POP are motes — replicas of host i % N with a
  // derived station (motes.mjs) — and they are ordinary droplets in every other
  // respect: same arrays, same forces, same pair interactions.
  //
  // POP === N is pre-R6 in every particular, which is what `?fmotes=0` selects.
  const POP = Math.max(N, Math.min(opts.pop ?? N, ballMax));
  const moteTab = POP > N ? makeMoteTable(POP, N, opts.temper ?? 1) : null;
  // How much of POP is currently DRAWN. The renderer's tier ladder moves this;
  // it is deliberately not an allocation, so a demotion never reallocates, never
  // strands physics state, and never makes a mote jump when the rung recovers —
  // every mote keeps being simulated, the demoted ones simply stop being packed.
  //
  // Motes are ordered by rank (all of rank 1 before any of rank 2), so lowering
  // this peels the OUTERMOST shell off each host first. That is the LOD the
  // ladder wants: the body thins from its surface inward instead of losing
  // whole regions.
  let activePop = POP;
  // Slots held back from the motes for the families that pack after them —
  // 12 ambient beads, the spray pool, and the scene extras (cursor chain, the
  // probe). Without it a dense composition would spend the whole buffer on
  // surface texture and silently delete the atmosphere and the pointer.
  const PACK_RESERVE = PHYS.AMBIENT_N + FLUID.SAT_POOL + 8;
  ...
  const P = new Float32Array(POP * 2);
  const R = new Float32Array(POP).fill(-1);
  // blended per-frame targets + physics attributes
  const TGT = new Float32Array(POP * 2);
  const TR = new Float32Array(POP);
  const BIND = new Float32Array(POP);
  const CLUS = new Int16Array(POP).fill(-1);
  const Z = new Float32Array(POP);
  ...
  const D = new Float32Array(POP).fill(1);
  ...
  // ── R7: THE MIST's hosts and environment ────────────────────────────────
  // The vapour condenses onto the AUTHORED droplets, addressed by identity —
  // never by packed slot, which shifts whenever a family enters or leaves the
  // buffer. (x, y, skin radius, presence) per host, rewritten at pack time
  // from the displayed positions, so the skin rides the same displacement the
  // bound droplets take. `mistEnv` packs what the droplets feel for the GPU.
  const hosts = new Float32Array(MIST_HOSTS * 4);
  const mistEnv = new Float32Array(8);
  const shockK =
    FLUID.SHOCK_A /
    (FLUID.FORM_SHOCK *
  ...
  const frame = (tMs, buf, aspect, zBuf, idBuf, dBuf) => {
    ...
    if (haveW) {
      ...
      // ── the motes (R6) ──────────────────────────────────────────────────
      // Derived from the hosts' ALREADY-BLENDED targets, so a mote inherits the
      // scene blend, the handoff weighting and the cluster arbitration for
      // free — one pass over the extra population instead of a second call into
      // every scene per replica. Radius and density ride the same low-pass the
      // hosts do, for the same reason: a radius that steps is a pop, and a
      // density that steps is a pop wearing a different name.
      if (moteTab) {
        const krM = 1 - Math.exp(-dt / PHYS.TAU_RADIUS);
        for (let i = N; i < POP; i++) {
          moteTarget(moteTab, i, t, TGT, TR, BIND, CLUS, Z, D);
          const ar = TR[i];
          if (R[i] < 0) R[i] = ar;
          else R[i] += (ar - R[i]) * krM;
        }
      ...
      if (fluid) {
        ...
        for (let i = 0; i < POP; i++) {
          const kp =
            1 - Math.exp(-dt / (TAUP[i % N] * (i < N ? 1 : moteTauScale(i))));
          if (!legacySeeded) {
            P[i * 2] = TGT[i * 2];
            P[i * 2 + 1] = TGT[i * 2 + 1];
          } else {
    ...
    // ── R7: the mist's block, and the hosts by identity ─────────────────────
    // The strongest present scene that breathes vapour owns the block; its
    // weight scales the master so a handoff fades the vapour with the scene.
    mistOut = null;
    let mistW = 0;
    for (let si = 0; si < scenes.length; si++) {
      if (W[si] < 1e-4 || !scenes[si].mist) continue;
      const b = scenes[si].mist(ctxs[si]);
      if (b && W[si] > mistW) {
        mistW = W[si];
        mistOut = b;
      }
    }
    mistOn = mistOut ? clamp01(mistOut.on) * mistW : 0;
    // Every authored host is written every frame — a culled droplet (too
    // small, too thin) keeps its position and reports presence 0, so its skin
    // is released rather than left orbiting a stale point.
    for (let i = 0; i < MIST_HOSTS && i < N; i++) {
      const o = i * 4;
      hosts[o] = P[i * 2];
      hosts[o + 1] = P[i * 2 + 1];
      const skin = mistOut ? mistOut.hostR[i] : R[i];
      hosts[o + 2] = skin > 0 ? skin : 0;
      hosts[o + 3] =
        R[i] >= 0.0012 && D[i] >= 0.004 && skin > 1e-4 ? clamp01(D[i]) : 0;
    }
    mistEnv[0] = env.px;
    mistEnv[1] = env.py;
    mistEnv[2] = env.pvx;
    mistEnv[3] = env.pvy;
    mistEnv[4] = env.pon ? 1 : 0;
    mistEnv[5] = env.press;
    mistEnv[6] = env.vel;
    mistEnv[7] = shockK;
    ...
    let depthMass = 0;
    let radiusMass = 0;
    const moteCeil = Math.max(N, ballMax - PACK_RESERVE);
    let moteCount = 0;
    let bindSum = 0;
    for (let i = 0; i < N; i++) bindSum += BIND[i];
    for (let i = 0; i < activePop; i++) {
      if (count >= ballMax) break;
      // the authored 48 are never traded away for anything
      if (i >= N && count >= moteCeil) break;
    ...
    if (hf) {
      ...
      lastForm.meltP = hf.meltP ?? -1;
      lastForm.sat = hf.sat ?? 0;
    } else {
    ...
    return {
      ...
      warp: lastForm.warp,
      meltP: lastForm.meltP,
      sat: lastForm.sat,
      mute: score.mute,
      count,
      // R6 diagnostics: how much of the packed buffer is motes, and the mean
      // bind that decided it. A mote count of 0 with a high bind is the system
      // working; a mote count of 0 with a low bind is a bug.
      motes: moteCount,
      bindAvg: bindSum / N,
      ...
      shock: shockU,
      touchLive,
      // R7: THE MIST — the dial block, its weight-scaled master, the hosts by
      // identity, the packed environment and the cached reading surfaces
      mist: mistOut,
      mistOn,
      hosts,
      mistEnv,
      obstacles: input.obstacles,
      obstacleCount: env.obstacleCount,
    };
  };
  ...
  return {
    ...
    driver: {
      ...
      frame,
      /** The simulated population, and how much of it is currently drawn. */
      population: { authored: N, simulated: POP, get active() { return activePop; } },
      /** Draw this many droplets. Clamped to [N, POP]: the authored population
       *  is a floor, because dropping one of those would drop a form endpoint. */
      setPopulation: (n) => {
        activePop = Math.max(N, Math.min(Math.floor(n) || N, POP));
        return activePop;
      },
    },
    raw,
