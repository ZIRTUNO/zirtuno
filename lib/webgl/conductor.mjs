...
export function makeConductor(scenes, opts = {}) {
  ...
  const input = { vel: 0 };
  const score = { exposure: 1, veil: 0, flash: 0, vignette: 0 };
  const stats = { violations: 0, holderId: null, energy: 0, active: 0 };
  ...
  // blended target attributes, kept for Phase B (physics) / Phase C (depth)
  ...
  const frame = (tMs, buf, aspect) => {
    ...
    if (haveW) {
      for (let i = 0; i < N; i++) {
        ...
        CLUS[i] = clus;
        Z[i] = az;
        // Phase A integrator — the legacy per-droplet low-pass (PHYS identity);
        // fluid-core.step replaces exactly this block in Phase B.
    ...
    // scene extras (cursor chain, probe, spray) — budget-enforced
    const push = (x, y, r) => {
    ...
    // ── light score (Phase D consumers; neutral until scenes score) ─────────
    score.exposure = 1;
    score.veil = 0;
    score.flash = 0;
    score.vignette = 0;
    ...
    // energy hint for the Phase C cadence governor
    let act = 0;
    for (let si = 0; si < scenes.length; si++) {
      if (W[si] < 1e-4) continue;
      const s = scenes[si];
    ...
    stats.energy = clamp01(act + Math.min(Math.abs(dmpVel) * 0.4, 0.4));
