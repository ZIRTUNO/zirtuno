...
float liquidField(vec2 uv) {
  ...
  float T = iFormA * formField(texture(iSDF, w).r)
          + iFormB * formField(texture(iSDF2, w).r);
  for (int i = 0; i < ${SDF_BALL_MAX}; i++) {
    if (i >= iBallCount) break;
    vec3 b = iBalls[i];
    vec2 dv = uv - b.xy;
    float d2 = max(dot(dv, dv), 4e-4);   // central-spike floor (matches field-shader)
