...
/**
 * THE MORPH'S SATURATION CEILING — how much every source beyond the strongest
 * may add, in total, WHILE A MORPH IS IN FLIGHT.
 *
 * This is deliberately not a page-wide setting, and the first attempt to make
 * it one is why: a single global law has to serve two families that want
 * opposite things. A morph's 48 droplets are a CROWD, and their pile-up is the
 * inflation. The confluence bodies, the Evolution ring and the cloud limbs are
 * held together BY that same overlap, and bounding it cost them 37-51% of
 * their bulk — they thinned and began showing their constituent droplets.
 *
 * They never share a frame. And a lone droplet is exact at every setting, so
 * the ambient family and the cursor cannot be affected by this at all. So the
 * ceiling rides the morph's own envelope (melt.mjs meltSat): the field is the
 * EXACT historical sum everywhere and at both endpoints of every morph, and
 * tightens only across the frames that were actually inflating.
 *
 * Measured mid-morph, mean worst circularity over the eight melts (the forms
 * themselves are 0.051):
 *
 *   MELT_SAT   mass dip   circ    holes   (baseline: 6.9%, 0.135, 0.1)
 *   off           6.9%     0.135    0.1
 *   1.5           8.0%     0.121    0.1
 *   1.0           9.2%     0.109    0.8
 *   0.8          10.5%     0.100    0.8     <- shipped
 *   0.6          12.8%     0.093    1.4     <- over the mass gate
 *   0.45         17.3%     0.087    2.9
 *
 * FORM_SOLIDITY is re-derived at each of these, so every row holds its own
 * mass: the ceiling stops the fattened droplets filling concavities while the
 * fattening puts the mid-morph mass back, which is why tightening it improves
 * shape instead of merely thinning the body. 0.8 is the tightest that still
 * clears melt-mass.mjs, and it also carries MORE holes and LESS pop than the
 * plain sum it replaces. Re-derive FORM_SOLIDITY (scripts/derive-form-
 * solidity.mjs) after any change here. `?fsat=<c>` overrides live for review;
 * `?fsat=0` is the exact rollback.
 */
export const MELT_SAT = 0.6;
