...
const BALL_LOOP = (withZ, withShape = false) => {
  ...
  for (int i = 0; i < ${SDF_BALL_MAX}; i++) {
    ...
    zw += fb * iBallZ[i];${accumV ? "\n    vw += fb * velocity;" : ""}`
        : `T += dens * (b.z * b.z) / d2 * win * formShield;`
    }
  }`;
};
...
uniform float iBallDensity[${SDF_BALL_MAX}];
...
${
  ...
  // The historical shader also added a bright core, Fresnel rim, and molten
  // specular. Those are deliberately not restored: the low-pass dome is the
  // useful technology; the shape-revealing decoration is not. Cyan remains the
  // ceiling, with a lightweight directional shadow below it and one page-level
  // lift shared by every connected or separated mass.
  vec3 cleanShadow = brand * 0.82;                      // same hue, light only
  vec3 cleanLight = mix(brand, vec3(0.120, 0.940, 1.000), 0.24);
  float softTone = 0.72 + 0.28 * smoothstep(0.22, 0.82, softDiff);
  vec3 domeCol = mix(cleanShadow, brand, softTone);
  vec2 keyAxis = normalize(L.xy);
  float fieldWash = smoothstep(-0.52, 0.52, dot(uv - 0.5, keyAxis));
  // The page-space component carries most of the contrast. The recovered dome
  // is deliberately only a supporting cue, which stops its normal from turning
  // the 48 source droplets into visible circles inside a fused body.
  vec3 pageCol = mix(brand * 0.76, cleanLight, 0.14 + 0.86 * fieldWash);
  vec3 cleanCol = mix(pageCol, domeCol, 0.18);
  ...
  float depthStrength = mix(0.10, 0.62, iGloss);
...
${
  ...
  float absorbStrength = iGloss;
  col /= vec3(1.0) + vec3(0.9, 0.45, 0.30) * (absorbStrength * iAbsorb * insideFx / (insideFx + 0.12));`
    : `  float absorbStrength = iGloss;
  col /= vec3(1.0) + vec3(0.9, 0.45, 0.30) * (absorbStrength * iAbsorb * inside / (inside + 0.12));`
}
  ...
  vec3 cleanKey = cleanLight * (0.05 + 0.07 * fieldWash + 0.02 * softDiff);
