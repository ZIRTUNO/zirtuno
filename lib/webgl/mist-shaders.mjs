...
void main() {
  ...
  if (p.x < exl) a.x += ${g(MIST.EDGE_A)} * clamp((exl - p.x) / ${g(MIST.EDGE_MARGIN)}, 0.0, 1.0);
  if (p.x > exr) a.x -= ${g(MIST.EDGE_A)} * clamp((p.x - exr) / ${g(MIST.EDGE_MARGIN)}, 0.0, 1.0);
  if (p.y < -${g(MIST.EDGE_MARGIN)}) a.y += ${g(MIST.EDGE_A)};
  if (p.y > ${g(1 + MIST.EDGE_MARGIN)}) a.y -= ${g(MIST.EDGE_A)};
