...
export const AURA = {
  ...
  DEPTH_DIM: 0.42,
  /**
   * THE OCCLUDERS — how many of the liquid's droplets the vapour is told about.
   *
   ...
   * moving the ground it exists to light by 2. That is backwards.
   *
   * So the vapour reads the liquid's own droplets and fades inside their field,
   * which is exactly what the mist did with its hosts (`bodyFade` in
   * `mist-shaders.mjs`) and the part of it that had to come back. 48 is the
   * mist's own figure and the authored population: the field is dominated by
   * the largest bodies, so the biggest 48 of whatever FieldStage packed are
   * enough, and the per-mote loop stays the size the mist already proved.
   */
  OCCLUDERS: 48,
  /**
   * Field value at which a mote starts to disappear, and where it is gone. The
   * liquid's iso-surface is at T = 1, so this clears a mote out just before the
   * surface and is fully gone just past it: a mote clinging to the rim of a
   * droplet is the same defect as one inside it, only harder to name.
   *
   * THESE CANNOT BE PUSHED MUCH LOWER, and the reason is that T is a SUM. Each
   * body's field has a long tail - windowed at seven radii - so forty-eight of
   * them contribute something almost everywhere near the composition. At 0.35
   * the aggregate of those tails cleared a third of the viewport: not a halo
   * around the liquid, a hard-edged void with no vapour in it at all, which is
   * a far worse artefact than the one it was fixing. The mist ran 0.85/1.35 on
...
// THE OCCLUDERS: the liquid's own droplets, already converted to this field's
// coordinates by the caller - x, y, radius, density. uOccN is how many are
// live; on a route with no liquid it is zero and the loop below never runs.
uniform vec4 uOcc[${AURA.OCCLUDERS}];
uniform int uOccN;
...
void main() {
  ...
  // THE LIQUID, TAKEN OUT OF THE VAPOUR. The same summed inverse-square field
  // the liquid shader itself evaluates, with the surface at T = 1, so "inside a
  // body" is decided by the body's real iso-surface and not by distance to any
  // one droplet - a merged body is fatter than the union of its circles,
  // because they neck. A mote is gone before it reaches that surface.
  float T = 0.0;
  for (int j = 0; j < ${AURA.OCCLUDERS}; j++) {
    if (j >= uOccN) break;
    vec4 H = uOcc[j];
    if (H.w < 0.02 || H.z < 1e-5) continue;
    vec2 dv = p - H.xy;
    float core = H.z * 0.18;
  ...
  float clear = 1.0 - smoothstep(${g(AURA.OCC_IN)}, ${g(AURA.OCC_OUT)}, T);
