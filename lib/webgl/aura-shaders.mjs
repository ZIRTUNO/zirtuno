...
export const AURA = {
  ...
  SIZE_LITE: 112,
  /**
   * The viewport the population above is authored for, in CSS pixels. A mote is
   * sized in CSS pixels, so a FIXED population means a small window gets the
   * same number of motes in a quarter of the area - measured on a 390x844
   * phone, four times the density of the desktop it was tuned on, which is a
   * different background rather than the same one smaller. The draw scales the
   * instance count by area against this, so what stays constant across a window
   * resize and across devices is the SPACING between motes.
   */
  REF_PX: 1440 * 900,
  /** …and the floor on that share, so a very small window still has weather. */
  MIN_SHARE: 0.22,
  ...
  ALPHA_VAR: 0.13,
  /**
   * THE SKEW on that spread — an exponent on the per-mote hash, so the
   * population's brightness is not uniformly distributed.
   *
   * Below 1 pushes mass toward the BRIGHT end, which is the owner's note that
   * the motes carrying more light should be more frequent. It is the same
   * device as R6's temperament and the opposite setting: that one wanted most
   * droplets calm with a short lively tail, and this wants the lively ones
   * common enough to be the field's character rather than its exception. At 1.0
   * the distribution is flat again.
   */
  ALPHA_SKEW: 0.55,
  /** Far motes dim by this share — the R5-C depth grade, at this scale. Eased
   *  back with the skew above: depth and skew both act on how many motes are
   *  faint, so leaving this where it was would have cancelled half the change. */
  DEPTH_DIM: 0.42,
  /**
   * THE MASK — how much of the liquid's own light hides a mote behind it.
   *
   * A background must not be visible THROUGH what stands in front of it, and
   * `mix-blend-mode: screen` does not deliver that: it attenuates by
   * (1 - backdrop), so over the liquid's mid-tone interior a mote still lands
   * at about half strength. Measured against the liquid's own frame-to-frame
   * churn, the vapour was moving the liquid's interior by ~7.6 levels while
   * moving the ground it exists to light by 2. That is backwards, and at a
   * zoom it is plainly dashes lying across the metaballs.
   *
   * THE MASK IS FIELDSTAGE'S CANVAS, SAMPLED - not a rebuild of it, and the
   * difference is the whole design. Rebuilding the metaball field here from the
   * droplet buffer FieldStage publishes was tried and fails twice over:
   *
   *   The liquid draws from up to 512 bodies and a uniform array cannot hold
   *   them. At the 48 largest, measured on a live frame, 22% of the body's area
   *   got no fade at all and another 20% only a partial one.
   *
   *   And it cannot work AT ALL on the SERVICE FORMS. At rest a form is an SDF
   *   texture with `count: 0` - literally zero droplets - so there is nothing
   *   to rebuild the shape from, over 60 000 to 90 000 pixels of solid liquid
   *   per frame across the whole Services chapter.
   *
   * Sampling the canvas is exact for droplets, motes, forms, melts and anything
   * that renderer ever draws, and it costs one fetch per mote. It needs
   * `preserveDrawingBuffer` on that context, which is set there with the
   * measurement; building a second renderer of the liquid in here to avoid it
   * is the thing AGENTS.md rule 15 exists to prevent.
   *
   * The signal is max(green, blue) rather than a perceptual luma: the liquid is
   * cyan, its red is near zero, and empty stage is bit-black (the post chain
   * gates its dither so it cannot seam against the page), which makes this a
   * clean presence test. IN is low because the liquid's dimmest interior is far
   * above it, and starting there is what makes a mote thin as it approaches an
   * edge instead of winking out on one.
   */
  MASK_IN: 0.03,
  MASK_OUT: 0.14,
...
// THE LIQUID, as it was actually drawn: FieldStage's canvas, uploaded by the
// caller every frame. uMaskOn is 0 on a route that has no liquid, and then this
// is never sampled.
...
void main() {
  ...
  // THE LIQUID, TAKEN OUT OF THE VAPOUR. One fetch of what FieldStage actually
  // painted at this mote's position - not a model of the liquid, the liquid.
  float clear = 1.0;
  if (uMaskOn > 0.5) {
    float presence = 0.0;
    vec3 lit = texture(uMask, vec2(p.x / uField.x, p.y)).rgb;
    presence = max(lit.g, lit.b);
    clear = 1.0 - smoothstep(${g(AURA.MASK_IN)}, ${g(AURA.MASK_OUT)}, presence);
  }

  // The brightness spread, skewed toward the bright end - see ALPHA_SKEW.
  float lit = pow(h1, ${g(AURA.ALPHA_SKEW)});
  vAlpha = uAlpha * edge * clear
         * (${g(AURA.ALPHA)} + ${g(AURA.ALPHA_VAR)} * lit)
