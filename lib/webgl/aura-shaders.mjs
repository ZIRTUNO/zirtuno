 ...
 * shapeless, because its shapes are its content. A cloud of specks 1.5 px
...
export const AURA = {
  /**
   * Texture edge per tier; the population is its square. 128^2 is 16 384 motes,
   * which over a 1440x900 viewport is one about every 9 px — dense enough to
   ...
   *
   * The window is narrower than it looks. The floor is the film grain, which
   * peaks around nine levels: below that a mote is not a particle, it is more
   * noise. The ceiling is that motes are drawn ADDITIVELY, so where two overlap
   * they sum — at 0.085 the brightest pixels measured 77, which on a black page
   * is a visible bright dot rather than a mote of dust.
   */
  ALPHA: 0.055,
  ALPHA_VAR: 0.05,
