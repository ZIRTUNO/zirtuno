...
// ── small math helpers ─────────────────────────────────────────────────────────
// NaN- and undefined-SAFE by construction. Written as `x < 0 ? 0 : ...` this
// returned its own input for NaN and for undefined, because both comparisons
// are false — so a renamed or misspelled channel produced NaN silently, flowed
// into the light score, and multiplied an entire fragment to nothing. Testing
// the positive case first makes 0 the answer for anything that is not a number,
// which is the identity for every channel that reads through here.
export const clamp01 = (x) => (x > 0 ? (x > 1 ? 1 : x) : 0);
...
export const AMB = Array.from({ length: PHYS.AMBIENT_N }, (_, k) => ({
  ...
  // Rates, not amplitudes. The beads used to run minutes-slow (a 48–126 s
  // period on x), which is indistinguishable from stillness over the seconds a
  // visitor actually spends in one chapter — so the atmosphere read as static
  // dots. Roughly doubling the rates covers the same ground on a timescale the
  // eye resolves; the amplitudes stay put because these beads are packed
  // straight into the conductor and never see the type-avoidance flow.
  f1: 0.1 + 0.15 * hash(k, 34), // x wander (rad/s)
  f2: 0.12 + 0.18 * hash(k, 35), // y buoyant loop
  f3: 0.21 + 0.24 * hash(k, 36), // y second octave
  f4: 0.26 + 0.34 * hash(k, 37), // radius breath
...
// The ecosystem's layout (canvas beads + SVG veins + DOM labels) lives in
// gathering.mjs since the S3 remake — "THE GATHERING".
