 ...
 *
 *   Mood remains legible through geometry: eye angles, asymmetry, tension,
 *   squash and body motion. Colour reinforces it with spring-driven weights.
 *
 ...
 *
 *   The owner-authorized emotion palette is scoped to the companion in CSS.
 *   This kernel emits only normalized weights; it owns no colour literals.
 *
...
import { EYE_POSES, EYE_POSE_NAMES, MOOD_SCORES, MOOD_NAMES, BASE_MOOD_NAMES, SPECIAL_MOOD_NAMES, BASE_EYE_POSE_NAMES, SPECIAL_EYE_POSE_NAMES } from "./companion-expressions.mjs";
export { EYE_POSES, EYE_POSE_NAMES, MOOD_SCORES, MOOD_NAMES, BASE_MOOD_NAMES, SPECIAL_MOOD_NAMES, BASE_EYE_POSE_NAMES, SPECIAL_EYE_POSE_NAMES };
 ...
 *   chill    depth on the cyan base; cool/warm/gold/blush/glow mix emotion light.
 */
const P = [
  "open",
  "squint",
...
const preset = (o) => {
  o = { leftW: .52, rightW: .52, leftH: 1, rightH: 1, tempo: 1, bodyX: 1, bodyY: 1, ...o };
...
// Legacy form states retain their semantic API; all use the redesigned eyes.
const legacyEyes = {
  rest: "small-attentive", notice: "upward-side-glance", attend: "attentive-left",
  read: "gentle-downward-gaze", ponder: "curious-left", doubt: "skeptical-right",
  approve: "joyful-wide", effort: "angry-right", hold: "small-attentive",
  fail: "uneasy-left", delivered: "eyes-closed", startled: "surprised-wide-left",
  dodge: "shy-downward",
};
for (const [name, eye] of Object.entries(legacyEyes)) {
  for (const [key, value] of Object.entries(EYE_POSES[eye])) {
    EXPRESSIONS[name][P.indexOf(key)] = value;
  }
}
const neutral = { open: 1, spread: 1, gaze: 1, pulse: 1, swell: 1, lean: .55, tension: .2 };
const vectors = {};
for (const [name, score] of Object.entries(MOOD_SCORES)) {
  vectors[name] = score.steps.map(([pose, ms]) => ({
    pose, ms, vector: preset({ ...neutral, ...EYE_POSES[pose], ...score.params }),
  }));
  EXPRESSIONS[name] = vectors[name][0].vector;
}
export const POSES = Object.freeze(Object.fromEntries(
  EYE_POSE_NAMES.map((name) => [name, preset({ ...neutral, ...EYE_POSES[name] })]),
));
...
export function makeCompanion(seed = 1) {
  ...
  let name = "rest";
  let playing = null;
  let poseName = legacyEyes.rest;
  let scoreIndex = 0;
  let scoreAt = 0;
  // Whole-body motion is one affine transform shared by body and pupils.
  let motionTilt = 0;
  let motionY = 0;
  let dirty = true;
  ...
  function integrate(h) {
    if (playing && t - scoreAt >= playing[scoreIndex].ms) {
      scoreAt += playing[scoreIndex].ms;
      scoreIndex = (scoreIndex + 1) % playing.length;
      poseName = playing[scoreIndex].pose;
      target.set(playing[scoreIndex].vector);
    }
  ...
  function build() {
    ...
    const phase = t / 1000 * Math.max(.1, v[PARAM.tempo]) * Math.PI;
    motionTilt = v[PARAM.rock] * Math.sin(phase);
    motionY = -v[PARAM.float] * 2.6 * (1 - Math.cos(phase)) / 2;
    const tilt = v[PARAM.tilt] + motionTilt;
    ...
    for (let i = 0; i < n; i++) {
      ...
      bx[i] = (x * ct - y * st) * v[PARAM.bodyX];
      by[i] = (x * st + y * ct) * v[PARAM.bodyY] + motionY;
    }
  }
  ...
  return {
    ...
    express(next) {
      const p = EXPRESSIONS[next] ?? POSES[next];
      name = p ? next : "rest";
      target.set(p ?? EXPRESSIONS.rest);
      playing = null;
      poseName = POSES[next] ? next : (vectors[next]?.[0].pose ?? legacyEyes[next] ?? legacyEyes.rest);
    },

    /** Run a mood's pose score. Repeated requests do not restart its clock. */
    play(next) {
      if (name === next && playing) return;
      if (!vectors[next]) { this.express(next); return; }
      name = next;
      playing = vectors[next];
      scoreIndex = 0;
      scoreAt = t;
      poseName = playing[0].pose;
      target.set(playing[0].vector);
    },
    ...
    pupilPath(side) {
      ...
      const width = Math.max(.12, v[side < 0 ? PARAM.leftW : PARAM.rightW]);
      const height = Math.max(.1, v[side < 0 ? PARAM.leftH : PARAM.rightH]) * openNow;
      const angle = v[side < 0 ? PARAM.leftAngle : PARAM.rightAngle] +
        v[PARAM.eyeSpin] * Math.sin(t / 350 + side * .3);
      const heart = clamp(v[side < 0 ? PARAM.heartL : PARAM.heartR], 0, 1);
      const star = clamp(v[side < 0 ? PARAM.starL : PARAM.starR], 0, 1);
      const diamond = clamp(v[side < 0 ? PARAM.diamondL : PARAM.diamondR], 0, 1);
      const ca = Math.cos(angle), sa = Math.sin(angle);
      const homeX = side * COMP.SPREAD * v[PARAM.spread] + v[PARAM.eyeX];
      const homeY = COMP.EYE_Y + v[PARAM.eyeY];
      const g = v[PARAM.gaze];
      containPupil(
        homeX + gx * COMP.GAZE_R * g,
        homeY + gy * COMP.GAZE_R * g,
        pr * Math.max(width, height) + Math.abs(v[PARAM.arch]) * pr * .55,
        v,
        radiusAt,
        hit,
      );
      ...
      const kA = 1 - COMP.FLINCH_A * flinch;
      const kP = 1 + COMP.FLINCH_A * flinch * 0.55;
      const rr = Math.min(width, height) * pr;
      const halfRun = Math.abs(width - height) * pr;
      const run = halfRun * 2;
      const arc = Math.PI * rr;
      const perimeter = 2 * (run + arc);

      for (let i = 0; i < pn; i++) {
        const b = (i / pn) * TAU;
        const cb = Math.cos(b);
        const sb = Math.sin(b);
        // A capsule is a circle swept along its longer axis. The same 26
        // vertices cover vertical pills, circles, horizontal lids and winks;
        // independent angle channels turn them into brows without extra parts.
        // Uniform ARC LENGTH also samples the straight runs. Sampling only
        // the end caps leaves no middle vertices to bend into a smiling lid.
        let distance = i / pn * perimeter;
        let ex, ey;
        if (distance < run) { ex = rr; ey = -halfRun + distance; }
        else if (distance < run + arc) {
          const angle = (distance - run) / rr;
          ex = rr * Math.cos(angle); ey = halfRun + rr * Math.sin(angle);
        } else if (distance < 2 * run + arc) {
          distance -= run + arc;
          ex = -rr; ey = halfRun - distance;
        } else {
          const angle = Math.PI + (distance - 2 * run - arc) / rr;
          ex = rr * Math.cos(angle); ey = -halfRun + rr * Math.sin(angle);
        }
        if (width > height) { const x = ex; ex = ey; ey = -x; }
        if (heart > 0) {
          const a = b + Math.PI / 2;
          const hx = Math.pow(Math.sin(a), 3) * .94 * width * pr;
          const hy = -(13 * Math.cos(a) - 5 * Math.cos(2*a) - 2 * Math.cos(3*a) - Math.cos(4*a)) / 18 * height * pr;
          ex += (hx - ex) * heart; ey += (hy - ey) * heart;
        }
        if (star > 0) {
          // An astroid gives the sparkle four pointed tips and concave sides.
          ex += (cb * cb * cb * .98 * width * pr - ex) * star;
          ey += (sb * sb * sb * .98 * height * pr - ey) * star;
        }
        if (diamond > 0) {
          const r = .94 / (Math.abs(cb) + Math.abs(sb));
          ex += (cb * r * width * pr - ex) * diamond;
          ey += (sb * r * height * pr - ey) * diamond;
        }
        ey += v[PARAM.arch] * pr * .55 * (1 - Math.pow(ex / (width * pr), 2));
        const x = cx + ex * ca - ey * sa;
        let y = cy + ex * sa + ey * ca;
        ...
        px[i] = (fx * ct - fy * st) * v[PARAM.bodyX];
        py[i] = (fx * st + fy * ct) * v[PARAM.bodyY] + motionY;
      }
