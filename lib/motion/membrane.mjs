...
export function buildRest(w, h, opts = {}) {
  ...
  const radius = Math.min(opts.radius ?? 0, w / 2, h / 2);
  if (radius > 0.01) return buildRoundRest(w, h, radius, seg);
  const L = 2 * (w + h);
...
/**
 * The rounded sibling of `buildRest`, and a different SURFACE rather than the
 * same one with softer corners.
 *
 * A sharp rectangle is four straight runs joined by cusps: the spline is told
 * to collapse its tangents at each corner so the edge turns ninety degrees,
 * and every deformation runs into a hard stop there. A rounded one has no
 * cusps at all — `sharp` is all zero — so the tension Laplacian is continuous
 * the whole way round and a wave launched on one edge carries into the next
 * instead of dying in a corner. That continuity is most of what reads as
 * SOFTNESS; the radius is only the part you can see.
 *
 * Arc length is what is sampled, not angle, and the arcs get the same step as
 * the straights: K_TEN is a wave speed in INDEX space, so equal index steps
 * must mean equal rest lengths or the corners quietly become stiffer than the
 * sides.
 */
function buildRoundRest(w, h, r, seg) {
  const arc = (Math.PI / 2) * r;
  const sw = Math.max(w - 2 * r, 0);
  const sh = Math.max(h - 2 * r, 0);
  const L = 2 * (sw + sh) + 4 * arc;

  let step = seg;
  let n = Math.round(L / step);
  for (let guard = 0; guard < 24 && (n < MEM.N_MIN || n > MEM.N_MAX); guard++) {
    step *= n > MEM.N_MAX ? 1.12 : 0.9;
    n = Math.round(L / step);
  }
  n = Math.max(MEM.N_MIN, Math.min(MEM.N_MAX, n));
  ...
  // The perimeter as one arc-length parameter, walked from the top-left
  // straight run clockwise. Corner centres are the four inset points.
  const cx = [r, w - r, w - r, r];
  const cy = [r, r, h - r, h - r];
  const segs = [
    { t: "s", len: sw }, // top
    { t: "a", len: arc, c: 1, a0: -Math.PI / 2 },
    { t: "s", len: sh }, // right
    { t: "a", len: arc, c: 2, a0: 0 },
    { t: "s", len: sw }, // bottom
    { t: "a", len: arc, c: 3, a0: Math.PI / 2 },
    { t: "s", len: sh }, // left
    { t: "a", len: arc, c: 0, a0: Math.PI },
  ];
  const straightAt = [
    { x0: r, y0: 0, dx: 1, dy: 0, nx: 0, ny: -1 },
    null,
    { x0: w, y0: r, dx: 0, dy: 1, nx: 1, ny: 0 },
    null,
    { x0: w - r, y0: h, dx: -1, dy: 0, nx: 0, ny: 1 },
    null,
    { x0: 0, y0: h - r, dx: 0, dy: -1, nx: -1, ny: 0 },
    null,
  ];
  ...
  for (let i = 0; i < n; i++) {
    let d = (i / n) * L;
    let k = 0;
    while (k < segs.length && d > segs[k].len) {
      d -= segs[k].len;
      k++;
    }
  ...
  return { n, w, h, L, bx, by, nx, ny, sharp: new Uint8Array(n), radius: r };
}
