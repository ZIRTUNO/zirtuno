...
export function makeMembrane(w = 200, h = 48, opts = {}) {
  ...
  const hand = (x, y, vx = 0, vy = 0) => {
    ...
    const ox = Math.max(-x, 0, x - rest.w);
    const oy = Math.max(-y, 0, y - rest.h);
  ...
  function substep(hSec, tMs) {
    ...
    for (let i = 0; i < n; i++) {
      dn[i] = clamp(dn[i] + vn[i] * hSec, -MEM.MAX_N, MEM.MAX_N);
  ...
  const path = (offset = 0) => {
    ...
    // Tangents. A Catmull-Rom tangent is (p[i+1] − p[i−1])/6; ZEROING it at a
    // vertex collapses that end's control point onto the vertex, which is the
    // standard way to put a CUSP in an otherwise smooth spline. That is what
    // keeps the four corners at ninety degrees: without it the spline rounds
    // each one over the 9 px to its neighbours, and the button reads as a
    // pill. Between two adjacent cusps both control points collapse and the
    // segment degenerates to a straight line — exactly right for an edge.
    const tx = new Array(n);
    const ty = new Array(n);
    const sharp = rest.sharp;
    for (let i = 0; i < n; i++) {
      if (sharp[i]) {
        tx[i] = 0;
        ty[i] = 0;
